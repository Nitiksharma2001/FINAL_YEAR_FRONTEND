    ...
    if(name.length > 0 && lastname.length > 0 && email.length > 0 && password.length > 0 && confirmPassword.length > 0 && role.length > 0){
      ...
      if(password === confirmPassword){
        ...
        const response = await axios.post("http://localhost:3000/api/v1/register", formData);
         toast.success("Addition of member successfull");
         navigate("/dashboard");
       }catch(err){
...
//   }, []);
  ...
  return (
      ...
      <div className="register-right">
        <div className="register-right-container">
          ...
          <div className="register-center">
            <h2>To add new members</h2>
            <p>Please enter their details</p>
            <form onSubmit={handleRegisterSubmit}>
            <input type="text" placeholder="Name" name="name" required={true} />
            <input type="text" placeholder="Lastname" name="lastname" required={true} />
              <input type="email" placeholder="Email" name="email" required={true} />

            <input type="text" placeholder="Role" name="role" required={true} />
    ...
    </div>:<div>Opening this route is not allowed</div>
  );
};
